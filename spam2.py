...
def verifyNum():
    num = int(input("How many people do you want to recieve this email? "))
    while(num < 1):
...
def emailTo(boolean):
    if(boolean):
        ...
        emails = input("Send email to who? ") 
        for i in range(0,numEmailSend-1):
            emails += "," + input("Send email to who? ")
    else:
        numEmailSend = int(input("How many people do you want to cc this email? "))
        if(numEmailSend < 1):
            return ""
        emails = input("Cc this email to who? ") 
        for i in range(0,numEmailSend-1):
            emails += "," + input("Cc this email to who? ")
    return emails
